...
export function ResumeAnalyzer() {
  ...
  const fileUploadAreaStyle = {
    background: dragActive 
      ? 'linear-gradient(145deg, rgba(139, 92, 246, 0.2), rgba(20, 184, 166, 0.2))'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
        <div>
          <h1 className="text-3xl bg-gradient-to-r from-purple-400 to-teal-400 bg-clip-text text-transparent">
            Resume Analyzer
          </h1>
          <p className="text-gray-400 mt-1">Upload and analyze resumes with AI-powered insights.</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm bg-blue-500/20 text-blue-300 border border-blue-500/30">
          ...
          <select 
            ...
            className="px-4 py-2 rounded-xl border border-white/20 text-gray-200 bg-white/5 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
            style={glassStyle}
          >
            {jobProfiles.map(profile => (
              <option key={profile.id} value={profile.id} className="bg-gray-800 text-gray-200">
                {profile.title}
              </option>
            ))}
          </select>
      ...
      {uploadedFiles.length > 0 && (
        <div className="rounded-2xl p-6 shadow-xl" style={glassStyle}>
          <h3 className="text-lg text-gray-200 mb-4">Uploaded Files</h3>
            ...
            {uploadedFiles.map((file, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-white/5 border border-white/10 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-gray-200 truncate max-w-48">{file.name}</p>
      ...
      {analysisResults.length > 0 && (
        <div className="space-y-6">
          <h2 className="text-2xl text-gray-200">Analysis Results</h2>
            ...
            <div key={result.id} className="rounded-2xl p-6 shadow-xl" style={glassStyle}>
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg text-gray-200">{result.fileName}</h3>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button 
                      ...
                      className="p-2 hover:bg-red-500/30 rounded-lg transition-colors text-red-400"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <h4 className="text-sm text-gray-400 mb-2">Name</h4>
                  <p className="text-gray-200">{result.analysis.personalInfo.name}</p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <h4 className="text-sm text-gray-400 mb-2">Email</h4>
                  <p className="text-gray-200 truncate">{result.analysis.personalInfo.email}</p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <h4 className="text-sm text-gray-400 mb-2">Phone</h4>
                  <p className="text-gray-200">{result.analysis.personalInfo.phone}</p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <h4 className="text-sm text-gray-400 mb-2">Location</h4>
                  <p className="text-gray-200">{result.analysis.personalInfo.location}</p>
                </div>
              </div>
                ...
                {result.analysis.sections.map((section, index) => (
                  <div key={index} className="bg-white/5 border border-white/10 rounded-xl p-4">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-sm text-gray-400">{section.name}</h4>
              ...
              {/* Skills */}
              <div className="mb-6">
                <h4 className="text-lg text-gray-200 mb-4">Detected Skills</h4>
              ...
              {/* Job Match */}
              <div className="bg-gradient-to-r from-purple-500/20 to-teal-500/20 border border-purple-500/30 rounded-xl p-4">
                <h4 className="text-lg text-gray-200 mb-3">Job Match Analysis</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <h5 className="text-sm text-gray-400 mb-2">Match Percentage</h5>
      ...
      {uploadedFiles.length === 0 && analysisResults.length === 0 && (
        <div className="text-center py-12">
          <div className="w-24 h-24 bg-gradient-to-br from-purple-500/20 to-teal-500/20 rounded-full flex items-center justify-center mx-auto mb-4">
            <FileSearch className="w-12 h-12 text-purple-400" />
          </div>
          <h3 className="text-xl text-gray-200 mb-2">No Resumes Uploaded</h3>
          <p className="text-gray-400">Upload your first resume to get started with AI-powered analysis.</p>
        </div>
      )}
    </div>
  );
